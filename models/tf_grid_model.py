...
# Meta_v1: update 2nd-to-last hidden state
class Meta_TF_Grid_Model_v1(tf.keras.Model, TF_Grid_Model):
    param_path = "models.tf_grid_model"
    param_name = "Meta_TF_Grid_Model_v1"
    ...
    def call(self, inputs):
        ...
        batch_cells = tf.concat([current_obs, current_hidden_state], -1)
        batch_cells = self.forward_step(batch_cells)
        # handle warmup timesteps from 1 to warmup_time-2
        for t in tf.range(1, self.warmup_time - 1):
            current_obs = warmup_obs[:, t, :, :]
            current_hidden_state = batch_cells[:, :, self.obs_dim:]
            batch_cells = tf.concat([current_obs, current_hidden_state], -1)
            batch_cells = self.forward_step(batch_cells)
        # handle warmup timestep warmup_time-1
        data_batch = {"pred_gt": warmup_obs[:, -1, :, :]}
        model_pred = batch_cells[:, :, :self.obs_dim]
        loss = self.meta_loss.get_loss(data_batch, model_pred)
        grad = tf.gradients(loss, current_hidden_state)
        print(grad)
